...
digitized_issues_year_state_total['Georgia'] = {}
...
for url in state_urls:
	...
	for issue in title_json['issues']:
		...
		day = issue_date[8:]
		if year in digitized_issues_year_state_total['Georgia']:
			digitized_issues_year_state_total['Georgia'][year] += 1
			total_issue_count += 1
		else:
			digitized_issues_year_state_total['Georgia'][year] = 1
			total_issue_count += 1
	...
	digitized_issues_year_state_total['Georgia']['total_issues'] = total_issue_count
	state_digitized_titles.append(title_information.copy())

# SORTS ISSUES BY YEAR
string_total_items_sorted = sorted(digitized_issues_year_state_total['Georgia'].items())

# OUTPUTS AS CSV FILE
with open('Georgia_total.csv', 'w', newline='') as csv_file:
    writer = csv.writer(csv_file)
    for key, value in string_total_items_sorted:
        writer.writerow(['Georgia', key, value])
