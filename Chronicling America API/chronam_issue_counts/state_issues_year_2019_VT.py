...
digitized_issues_year_state_total['Vermont'] = {}
...
for url in state_urls:
	...
	for issue in title_json['issues']:
		...
		day = issue_date[8:]
		if year in digitized_issues_year_state_total['Vermont']:
			digitized_issues_year_state_total['Vermont'][year] += 1
			total_issue_count += 1
		else:
			digitized_issues_year_state_total['Vermont'][year] = 1
			total_issue_count += 1
	...
	digitized_issues_year_state_total['Vermont']['total_issues'] = total_issue_count
	state_digitized_titles.append(title_information.copy())

# SORTS ISSUES BY YEAR
string_total_items_sorted = sorted(digitized_issues_year_state_total['Vermont'].items())

# OUTPUTS AS CSV FILE
with open('Vermont_total.csv', 'w', newline='') as csv_file:
    writer = csv.writer(csv_file)
    for key, value in string_total_items_sorted:
        writer.writerow(['Vermont', key, value])
