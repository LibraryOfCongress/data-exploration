...
digitized_issues_year_state_total['Oklahoma'] = {}
...
for url in state_urls:
	...
	for issue in title_json['issues']:
		...
		day = issue_date[8:]
		if year in digitized_issues_year_state_total['Oklahoma']:
			digitized_issues_year_state_total['Oklahoma'][year] += 1
			total_issue_count += 1
		else:
			digitized_issues_year_state_total['Oklahoma'][year] = 1
			total_issue_count += 1
	...
	digitized_issues_year_state_total['Oklahoma']['total_issues'] = total_issue_count
	state_digitized_titles.append(title_information.copy())

# SORTS ISSUES BY YEAR
string_total_items_sorted = sorted(digitized_issues_year_state_total['Oklahoma'].items())

# OUTPUTS AS CSV FILE
with open('Oklahoma_total.csv', 'w', newline='') as csv_file:
    writer = csv.writer(csv_file)
    for key, value in string_total_items_sorted:
        writer.writerow(['Oklahoma', key, value])
